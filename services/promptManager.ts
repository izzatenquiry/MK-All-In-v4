...
// --- Angle Photos Prompt ---
export const getAnglePhotosPrompt = (details: {
  gender: string;
  modelFace: string;
  hasFaceImage?: boolean;
  ...
  const { vibe, style, lighting, composition, lensType, filmSim, creativityLevel } = details.creativeDirection;
  
  const faceInstruction = details.hasFaceImage 
    ? `A ${details.gender} model. The person's face in the final image **MUST be a photorealistic and exact match** to the face from the provided face reference image. **Do not alter** their facial features, structure, or identity. The final image must look like it features the same person.`
    : `A ${details.gender} model with facial features typical of ${details.modelFace === 'Random' ? 'Southeast Asia' : details.modelFace}.`;

  const productInstruction = details.hasProductImage 
    ? "Naturally integrate the product into the scene."
    : "Create a photorealistic UGC-style scene without a product.";

  const promptParts = [
    `You are an AI image generation expert. Create a photorealistic UGC-style photo${details.hasProductImage ? ' showing a model interacting with the provided product' : ''}.`,
    ``,
    `**SCENE REQUIREMENTS:**`,
    `- ${faceInstruction}`,
    `- **Camera Angle:** ${details.camera} (CRITICAL: Strictly follow this camera perspective)`,
    `- **Model Pose:** ${details.pose} (CRITICAL: Strictly follow this body posture)`,
    `- **Product Integration:** ${productInstruction}`,
    ``,
    `**AESTHETIC SETTINGS:**`,
    `- Background/Vibe: ${vibe}`,
    `- Artistic Style: ${style === 'Random' ? 'photorealistic' : style}`,
    `- Lighting: ${lighting === 'Random' ? 'natural cinematic lighting' : lighting}`,
    `- Composition: ${composition === 'Random' ? 'centered and balanced' : composition}`,
    `- Lens: ${lensType === 'Random' ? 'standard lens' : lensType}`,
    `- Look: ${filmSim === 'Random' ? 'modern and crisp' : filmSim}`,
  ...
  promptParts.push(
    ``,
    `**CRITICAL:** No text, no watermarks. Purely visual high-quality TikTok/Instagram Reels style frame.`
  );
