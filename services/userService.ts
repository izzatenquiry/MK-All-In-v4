...
export const loginUser = async (email: string): Promise<LoginResult> => {
    ...
    if (userData && !userError) {
        ...
        // ✅ Check subscription expiry (untuk kedua-dua brand)
        if (user.status === 'subscription' && user.subscriptionExpiry) {
            const now = Date.now();
            if (user.subscriptionExpiry < now) {
                return { success: false, message: 'subscriptionExpired' };
            }
        }
        
        // Note: MONOKLIX users with expired Token Ultra can still login
        // Token Ultra expiry is checked at token generation and AI generation level instead
...
export const getTokenUltraRegistration = async (
  ...
  try {
    ...
    if (status !== calculatedStatus) {
      ...
      if (updateError) {
        console.warn('Failed to update token ultra registration status:', updateError);
