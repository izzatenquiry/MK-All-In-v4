  ...
  const fetchData = async () => {
    ...
    try {
      ...
      const enhancedUsers: EnhancedUser[] = (supabaseUsers || []).map(user => {
        ...
        // Standardized: expires_at sudah di-populate dalam mapProfileToUser untuk kedua-dua brand
        return {
          ...user,
          registered_at: user.registered_at || user.createdAt, // Use registered_at from users table
          expires_at: user.expires_at, // Standardized untuk kedua-dua brand
          ...
          registration: reg || undefined, // Keep for backward compatibility if needed
          app_version: user.appVersion, // Use directly from user
          last_device: user.lastDevice || undefined, // Use directly from user
          proxy_server: user.proxyServer || undefined, // Use directly from user
          personal_auth_token: user.personalAuthToken || undefined, // Use directly from user
          last_seen_at: user.lastSeenAt, // Use directly from user
