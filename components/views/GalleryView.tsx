...
const GalleryView: React.FC<GalleryViewProps> = ({ onCreateVideo, onReEdit, language }) => {
    ...
    const downloadAsset = (item: HistoryItem) => {
        ...
        const brandPrefix = BRAND_CONFIG.shortName.toLowerCase();
    
        switch (item.type) {
            case 'Image':
            case 'Canvas':
                fileName = `${brandPrefix}-${item.type.toLowerCase()}-${item.id}.png`;
                href = `data:image/png;base64,${item.result}`;
                break;
            case 'Video':
            case 'Audio':
                const extension = item.type === 'Video' ? 'mp4' : 'wav';
                fileName = `${brandPrefix}-${item.type.toLowerCase()}-${item.id}.${extension}`;
                href = blobUrls.get(item.id) || null;
                break;
            case 'Storyboard':
            case 'Copy':
                fileName = `${brandPrefix}-${item.type.toLowerCase()}-${item.id}.txt`;
