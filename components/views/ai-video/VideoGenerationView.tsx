...
const VideoGenerationView: React.FC<VideoGenerationViewProps> = ({ preset, clearPreset, currentUser, onUserUpdate, language }) => {
  ...
  const handleGenerate = useCallback(async () => {
      ...
      try {
          ...
          // Check if it's a token error and preserve the full message
          if (errorMessage.includes('ERROR 401') || errorMessage.includes('token is invalid') || 
              errorMessage.includes('has expired') || e instanceof Error && 
